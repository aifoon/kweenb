   ...
   private static getPozyxMqttBrokerInstance(pozyxMqttBrokerUrl?: string, force?: boolean) {
    if((!this._pozyxMqttBroker && pozyxMqttBrokerUrl) || (pozyxMqttBrokerUrl && force)) {
      this._pozyxMqttBroker = new MQTT(pozyxMqttBrokerUrl, {
        ...
        onMessage: (topic, message) => message && topic && this.messageHandler(topic, message)
      });
    }
   ...
   private static messageHandler(topic: string, message: any) {
    switch(topic) {
      case "tags": {
        ...
        KweenBGlobal.kweenb.mainWindow.webContents.send("pozyx-data", PozyxData.getAllPozyxData());

        // let the positioning controller know that we have new data
        PositioningControllerSingleton.getInstance().positioningUpdateReceived(PozyxData.getAllPozyxData());
